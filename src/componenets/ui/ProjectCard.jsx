     ...
     <div className="group relative bg-white/5 border-white/10 rounded-2xl overflow-hidden hover:border-primary/30 transition-all duration-300">
      ...
      <div className="p-6 space-y-4">
        <div>
          ...
          <p className="text-white/60 ext-sm leading-relaxed line-clamp-2">
            {description}
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {technologies.map((tech, index) => (
            <span key={index} className="px-3 p-1 text-xs font-medium text-primary bg-primary/10 border border-primary/20 rounded-lg hover:bg-primary/20 transition-colors duration-200">
              {tech}
            </span>
          ))}
        </div>
